...
class VariantionalAutoencoder(object):
    ...
    def build(self):
        ...
        self.z_log_sigma_sq = fc(
            f3, self.n_z, scope='enc_fc4_sigma', activation_fn=None)
        ...
        self.x_hat = fc(
            g3, input_dim, scope='dec_fc4', activation_fn=tf.sigmoid)
    ...
    def run_single_step(self, x):
        _, loss, recon_loss, latent_loss = self.sess.run(
            [
                self.train_op, self.total_loss, self.recon_loss,
                self.latent_loss
            ],
            feed_dict={self.x: x})
...
for i in range(n):
    for j in range(n):
        I_generated[i * h:(i + 1) * h, j * w:(
            j + 1) * w] = x_generated[i * n + j, :].reshape(28, 28)
...
for i, yi in enumerate(x):
    for j, xi in enumerate(y):
        ...
        I_latent[(n - i - 1) * 28:(n - i) * 28, j * 28:(
            j + 1) * 28] = x_hat[0].reshape(28, 28)
